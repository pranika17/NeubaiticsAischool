...
const MiniVideoPlayer = () => {
 ...
 const handleTimeUpdate = async () => {
  ...
  const getVideoUrl = (chapter) =>
    chapter?.video_stream_url || chapter?.video_url || chapter?.video || "";
  ...
  const isEmbeddableVideo = (url = "") => Boolean(getYouTubeEmbedUrl(url));

  const markCurrentChapterCompleted = async () => {
    if (!currentVideo) return;

    await sendProgressToBackend(currentVideo.id, 1, 1);
    setProgressMap((prev) => ({
      ...prev,
      [currentVideo.id]: {
        is_completed: true,
    ...
    Swal.fire({
      ...
      text: "This external video chapter is marked completed.",
      icon: "success",
      toast: true,
      position: "top-right",
      timer: 2500,
    ...
    <div className="video-page">
      <div className="video-layout">
        ...
        <div className="video-left">
          {currentVideo ? (
            <>
              {isEmbeddableVideo(getVideoUrl(currentVideo)) ? (
                <>
                  <iframe
                    key={currentVideo.id}
                    src={getYouTubeEmbedUrl(getVideoUrl(currentVideo))}
                  ...
                  <div className="video-controls">
                    <button onClick={markCurrentChapterCompleted}>
                      Mark Completed
                    </button>
                  </div>
                </>
            ...
            chapters.map((ch, index) => (
              ...
              >
                <div className="playlist-thumb">
                  {isEmbeddableVideo(getVideoUrl(ch)) ? (
                    <div className="external-video-thumb">
                      <i className="bi bi-play-circle-fill"></i>
                    </div>
                  ) : (
                    <video src={getVideoUrl(ch)} muted />
                  )}
                </div>
