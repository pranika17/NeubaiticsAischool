  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((res) => {
        ...
        setCourseData(rows.filter((item) => item?.status === "approved"));
      })
                  ...
                  <div key={course.id} className="course-card popup-course-card">
                    ...
                    <div className="course-actions mt-3">
                      ...
                      {eligible ? (
                        approved ? (
                          <button
                            className="glass-btn certificate-action-btn"
                            onClick={() => handleCertificate(course.id)}
                          >
                            🎓 {pdf ? "Download" : "Generate"}
                          </button>
                        ) : (
                          <button className="glass-btn disabled-btn certificate-action-btn" disabled>
                            ⏳ Waiting Approval
                          </button>
                        )
                      ) : (
