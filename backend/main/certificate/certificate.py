...
def calculate_course_progress(student, course):
    ...
    assignments_done = (total_assignments == 0 or total_assignments == completed_assignments)

    # ✅ Quiz
    quiz_passed = False
    quiz_score = 0

    cq = CourseQuiz.objects.filter(course=course).first()
    if cq and cq.quiz:
        total_questions = QuizQuestions.objects.filter(quiz=cq.quiz).count()
        if total_questions > 0:
            correct = AttemptQuiz.objects.filter(student=student, quiz=cq.quiz, is_correct=True).count()
            quiz_score = round((correct / total_questions) * 100)
            quiz_passed = quiz_score >= PASS_MARK
    ...
    return {
        ...
        "quiz": {"score": quiz_score, "ok": quiz_passed},
