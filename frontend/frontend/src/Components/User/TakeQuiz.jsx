...
const TakeQuiz = () => {
  ...
  useEffect(() => {
    ...
    axios.get(`${baseUrl}/fetch-quiz-attempt-status/${quiz_id}/${studentId}`)
      .then((statusRes) => {
        const status = statusRes.data || {};
        if (status.completed || status.bool) {
          setCompletedStatus(status);
          setQuizFinished(true);
          return null;
        }

        return axios.get(`${baseUrl}/quiz-questions/${quiz_id}`, {
      ...
      .catch(err => {
        if (err.response?.data?.completed) {
          setCompletedStatus(err.response.data);
          setQuizFinished(true);
          return;
        }
    ...
    axios.get(`${baseUrl}/quiz-questions/${quiz_id}/next-question/${currentQuestionId}`, {
      ...
      .then(res => {
        ...
        if (!questions || questions.length === 0 || res.data.finished) {
          ...
          setQuestionData([]);
          axios
            .get(`${baseUrl}/fetch-quiz-attempt-status/${quiz_id}/${studentId}`)
            .then((statusRes) => setCompletedStatus(statusRes.data || null))
            .catch(() => {});
        } else {
  ...
  if (quizFinished) {
    const score = Number(completedStatus?.score || 0);
    const passMark = Number(completedStatus?.pass_mark || 60);
    const passed = Boolean(completedStatus?.passed);

    return (
      <div className="quiz-finish-box text-center">
        <h3>🎉 Quiz Completed!</h3>
        {completedStatus && (
          <div className={`quiz-completed-status ${passed ? "pass" : "fail"}`}>
