...
def teacher_login(request):
    ...
    try:
        ...
        login_id = str(data.get("login_id") or data.get("email", "")).strip()
        password = data.get("password")
    except:
        return JsonResponse({"bool": False, "msg": "Invalid request"})

    if not login_id:
        return JsonResponse({"bool": False, "msg": "Teacher ID or email is required."})

    login_lookup = Q(teacher_code__iexact=login_id)
    try:
        validate_email(login_id)
        login_lookup |= Q(email__iexact=login_id.lower())
    except DjangoValidationError:
        pass

    teacher = Teacher.objects.filter(login_lookup).first()
    if not teacher:
...
class CourseList(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        # --- Popular filter ---
        if 'popular' in self.request.GET:
            final_ordering = ('-course_views', '-id')
...
class StudentRegisterView(generics.ListCreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        student = serializer.save(is_approved=False)
...
def student_login(request):
    ...
    try:
        ...
        login_id = request.POST.get("login_id", "").strip() or request.POST.get("email", "").strip()
        password = request.POST.get("password", "").strip()

    if not login_id:
        return JsonResponse({"bool": False, "msg": "Student ID or email is required."})

    login_lookup = Q(student_code__iexact=login_id)
    try:
        validate_email(login_id)
        login_lookup |= Q(email__iexact=login_id.lower())
    except DjangoValidationError:
    ...
    student = Student.objects.filter(login_lookup).first()
...
class AssignmentList(generics.ListCreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        if course and course.teacher_id != teacher.id:
            raise serializers.ValidationError({"course": "This course does not belong to the teacher."})

        if course and not models.StudentCourseEnrollment.objects.filter(
            student=student,
            course=course,
            status="approved"
        ).exists():
            raise serializers.ValidationError({"course": "Student is not approved for this course."})
...
class SubmitAssignment(generics.UpdateAPIView):
    ...
    def patch(self, request, *args, **kwargs):
        ...
        has_file = bool(request.FILES.get("upload_file"))
        has_image = bool(request.FILES.get("upload_image"))

        if not answer_text and not has_file and not has_image:
            return Response(
                {"error": "Please provide an answer, file, or image before submitting."},
                status=status.HTTP_400_BAD_REQUEST
            )
        ...
        # save fields
        assignment.answer_text = answer_text or assignment.answer_text
...
class QuizQuestionList(generics.GenericAPIView):
    ...
    def get(self, request, quiz_id, question_id=None, limit=None):
        ...
        if student_id:
            ...
            summary = _get_quiz_attempt_summary(student_id_int, quiz_id)
...
class AttempQuizList(generics.ListCreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        summary = _get_quiz_attempt_summary(student_id, quiz_id)
        if summary["completed"]:
            return JsonResponse(
                {
                    "error": "Quiz already completed.",
...
def fetch_quiz_attempt_status(request,quiz_id,student_id):
    ...
    return JsonResponse(_get_quiz_attempt_summary(student_id, quiz_id))
...
def fetch_quiz_result(request,quiz_id,student_id):
    ...
    total_attempted = summary["total_attempted_questions"]
...
def _get_quiz_attempt_summary(student_id, quiz_id):
    total_questions = models.QuizQuestions.objects.filter(quiz_id=quiz_id).count()
    attempts = models.AttemptQuiz.objects.filter(student_id=student_id, quiz_id=quiz_id)
    total_attempted = attempts.values("question_id").distinct().count()
    total_correct = attempts.filter(is_correct=True).count()
    total_wrong = total_attempted - total_correct
    total_unattempted = max(total_questions - total_attempted, 0)
    score = round((total_correct / total_questions) * 100) if total_questions else 0
    completed = total_questions > 0 and total_attempted >= total_questions

    return {
        "bool": completed,
        "completed": completed,
        "passed": completed and score >= QUIZ_PASS_MARK,
        "score": score,
...
def student_assigned_quizzes(request, student_id):
    ...
    for row in serializer.data:
        item = dict(row)
        item["attempt_status"] = _get_quiz_attempt_summary(student_id, item["id"])
        data.append(item)
...
def _fallback_interview_questions(course, interview_type):
    ...
    question_bank = [
        ...
        {
            "round_type": "intro",
            "question_text": f"Give a simple overview of {title} as if you were explaining it to a hiring manager.",
            "ideal_points": f"Explain the course goal, the core basics of {techs}, and what kind of problems it solves.",
            "coding_prompt": "",
        },
        {
            "round_type": "technical",
            "question_text": f"What are the basic concepts from {title} that every beginner must understand before building real projects?",
            "ideal_points": f"Cover the foundation topics from {techs}, why they matter, and one practical example.",
            "coding_prompt": "",
        },
        {
            "round_type": "technical",
        ...
        {
            "round_type": "technical",
            "question_text": f"Describe one development challenge you may face while building with {techs} and how you would solve it.",
            "ideal_points": "Real problem, debugging approach, tradeoffs, and implementation decision.",
            "coding_prompt": "",
        },
        {
            "round_type": "coding",
...
def start_mock_interview(request):
    ...
    prompt = (
        ...
        "This must feel like a real interview for an enrolled student preparing to build projects and attend hiring rounds. "
        "Include a realistic sequence of self-introduction, basic/fundamental questions, practical development questions, and coding where appropriate. "
        "Every question must be based on the student's enrolled course, studied content, assignments, quizzes, and project readiness. "
        "Ask what they would build, how they would develop it, and how they would solve real implementation problems. "
        "Include coding only when the course clearly supports it. Keep questions job-oriented, course-aware, and beginner-to-intermediate realistic."
    )
...
def submit_mock_interview_answer(request):
    ...
    if not interview_id or not question_id:
        return Response({"error": "interview_id and question_id are required"}, status=400)
    ...
    if not answer_text:
        answer_text = "No response provided within the allotted interview wait time."
...
If the student gave no answer or an incomplete answer, score accordingly and clearly say what they should have said.
Keep the response simple, easy to understand, practical, and interview-like.
Summarize what the student actually said, what was missing, and provide a better sample answer in plain language instead of a theoretical explanation.
"""
    evaluation = evaluate_mock_interview_answer(evaluation_prompt, context_chunks)
    if "error" in evaluation:
        evaluation = {
            "answer_summary": "You gave a partial answer, but it did not clearly cover the main point of the question.",
            "score": 55,
            "communication_score": 55,
            "technical_score": 55,
            "confidence_score": 55,
            "feedback": "Your answer covers some points but needs a clearer structure and more practical examples.",
            "improvement_tip": "Use a simple flow: direct answer, short explanation, one real example, and final result.",
            "missing_points": "You missed the most important concept, a project example, and a confident closing line.",
            "sample_answer": "A stronger answer would briefly define the concept, explain where you used it in your course or project, and mention the outcome in simple words.",
...
QUIZ_DUPLICATE_STOP_WORDS = {
    "about", "after", "again", "also", "answer", "based", "basic", "before",
    "best", "course", "correct", "detail", "does", "each", "following",
    "from", "generate", "given", "important", "include", "learners", "level",
    "match", "multiple", "option", "problem", "program", "question", "quiz",
    "should", "student", "teacher", "thing", "topic", "used", "using", "what",
    "when", "where", "which", "with", "write", "would", "without", "existing",
    "questions", "level", "make", "create", "give",
}


def _clean_quiz_text(value):
    return re.sub(r"[^a-z0-9]+", " ", str(value or "").lower()).strip()


def _quiz_signature_tokens(value):
    tokens = _clean_quiz_text(value).split()
    return {
        token
        for token in tokens
        if len(token) >= 4 and token not in QUIZ_DUPLICATE_STOP_WORDS
    }


def _quiz_concept_signature(value):
    tokens = sorted(_quiz_signature_tokens(value))
    return " ".join(tokens[:14])


def _extract_requirement_keywords(teacher_prompt):
    cleaned = _clean_quiz_text(teacher_prompt)
    if not cleaned:
        return []
    ...
    words = [
        ...
        if len(word) >= 3 and word not in QUIZ_DUPLICATE_STOP_WORDS
    ]
    seen = []
    for word in words:
        if word not in seen:
...
def _required_requirement_topic(teacher_prompt):
    keywords = _extract_requirement_keywords(teacher_prompt)
    return " ".join(keywords[:6])


def _quiz_question_similarity(left, right):
    left_clean = _clean_quiz_text(left)
    right_clean = _clean_quiz_text(right)
    if not left_clean or not right_clean:
        return 0

    left_words = set(left_clean.split())
    right_words = set(right_clean.split())
    left_signature = _quiz_signature_tokens(left)
    right_signature = _quiz_signature_tokens(right)
    overlap = len(left_words & right_words) / max(1, len(left_words | right_words))
    signature_overlap = 0
    if left_signature and right_signature:
        signature_overlap = len(left_signature & right_signature) / max(1, min(len(left_signature), len(right_signature)))
    ratio = SequenceMatcher(None, left_clean, right_clean).ratio()
    return max(overlap, signature_overlap, ratio)


def _is_duplicate_quiz_question(candidate, existing_questions, threshold=0.68):
    candidate_clean = _clean_quiz_text(candidate)
    if not candidate_clean:
        return True

    for existing in existing_questions:
        if candidate_clean == _clean_quiz_text(existing):
            return True
        if _quiz_question_similarity(candidate, existing) >= threshold:
            return True
    return False


def _existing_quiz_concepts(existing_questions):
    concepts = []
    for question in existing_questions[-80:]:
        signature = _quiz_concept_signature(question)
        if signature and signature not in concepts:
            concepts.append(signature)
    return concepts


def _select_quiz_generation_angle(quiz, course, existing_questions, difficulty, focus, attempt_number=1, teacher_prompt=""):
    existing_count = len(existing_questions)
    base_topics = []
    requirement_topic = _required_requirement_topic(teacher_prompt)

    if requirement_topic:
        base_topics.append(requirement_topic)

    if course:
        base_topics.extend([
            getattr(course, "title", ""),
            getattr(course, "techs", ""),
        ])
        base_topics.extend(
        ...
        base_topics.extend(
            models.StudyMaterial.objects
            .filter(course=course)
            .order_by("-id")
            .values_list("title", flat=True)[:8]
        )

    base_topics.extend([getattr(quiz, "title", ""), getattr(quiz, "detail", "")])
    base_topics = [str(topic).strip() for topic in base_topics if str(topic or "").strip()]

    if not base_topics:
        base_topics = ["core concept", "practical use", "debugging", "real-world application"]
    ...
    if requirement_topic:
        ...
        topic = base_topics[(existing_count + attempt_number - 1) % len(base_topics)]
    angle = angle_templates[(existing_count + attempt_number - 1) % len(angle_templates)]
    return f"{difficulty} {focus}: {topic} - {angle}"


def _get_quiz_course(quiz):
    assignment = (
        models.CourseQuiz.objects
        .select_related("course")
        .filter(quiz=quiz, course__isnull=False)
        .order_by("-add_time")
        .first()
    )
    return assignment.course if assignment else None


def _build_quiz_context_chunks(quiz, course=None):
    context_chunks = [
    ...
    if course:
        context_chunks.extend([
            ...
            f"Course Description: {course.description}",
            f"Course Technologies: {course.techs or ''}",
        ])

        chapter_lines = [
            f"{chapter.title}: {chapter.description or chapter.remarks or ''}"
            for chapter in models.Chapter.objects.filter(course=course).order_by("id")[:10]
        ]
        if chapter_lines:
            context_chunks.append("Course Chapters:\n" + "\n".join(chapter_lines))

        material_lines = [
            f"{material.title}: {material.description or material.remarks or ''}"
            for material in models.StudyMaterial.objects.filter(course=course).order_by("-id")[:8]
        ]
        if material_lines:
            context_chunks.append("Study Materials:\n" + "\n".join(material_lines))

    return context_chunks


def _normalize_quiz_generation_options(request, quiz):
    raw_type = str(request.data.get("question_type", "mcq") or "mcq").strip().lower()
    raw_difficulty = str(request.data.get("difficulty", "") or "").strip().lower()
    raw_focus = str(request.data.get("focus", "") or request.data.get("category", "") or "").strip().lower()
    prompt = str(request.data.get("prompt", "") or "").strip()

    if raw_type in {"code", "programming"}:
        raw_type = "coding"
    if raw_type not in {"mcq", "coding"}:
        raw_type = "coding" if "coding" in prompt.lower() or "program" in prompt.lower() else "mcq"

    prompt_lower = prompt.lower()
    if not raw_difficulty:
        if any(word in prompt_lower for word in ["advanced", "advance", "hard", "expert", "difficult"]):
            raw_difficulty = "advanced"
        elif any(word in prompt_lower for word in ["intermediate", "medium"]):
            raw_difficulty = "intermediate"
        else:
            raw_difficulty = "basic"

    allowed_difficulties = {"basic", "intermediate", "advanced"}
    difficulty = raw_difficulty if raw_difficulty in allowed_difficulties else "basic"

    if not raw_focus:
        raw_focus = "coding problem" if raw_type == "coding" else "concept understanding"

    title_hint = (quiz.title or "").strip()
    detail_hint = (quiz.detail or "").strip()
    teacher_instruction = prompt or f"Generate a {difficulty} {raw_focus} question for {title_hint or detail_hint}."

    return raw_type, difficulty, raw_focus, teacher_instruction


def _build_existing_question_text(existing_questions):
    if not existing_questions:
        return "No existing questions yet."
    selected = existing_questions[-60:]
    return "\n".join(f"- {question}" for question in selected)


def _request_generated_history(request):
    raw_history = (
        request.data.get("generated_history")
        or request.data.get("recent_questions")
        or request.data.get("avoid_questions")
        or []
    )
    ...
    if not isinstance(raw_history, list):
        return []
    return [str(question).strip() for question in raw_history if str(question or "").strip()]


def _build_quiz_generation_prompt(quiz, course, question_type, difficulty, focus, teacher_prompt, existing_questions, attempt_number):
    course_title = getattr(course, "title", "") or "the assigned course"
    existing_text = _build_existing_question_text(existing_questions)
    existing_concepts = _existing_quiz_concepts(existing_questions)
    existing_concept_text = "\n".join(f"- {concept}" for concept in existing_concepts[-40:]) or "No used concepts yet."
    generation_angle = _select_quiz_generation_angle(
        quiz=quiz,
        course=course,
    ...
    requirement_topic = _required_requirement_topic(teacher_prompt)
    requirement_rule = (
        f"- The question MUST primarily test this typed requirement/topic: {teacher_prompt}.\n"
        f"- Required keywords/concepts that should appear in the generated question or options: {requirement_topic}.\n"
        if requirement_topic
        else "- If the teacher instruction is generic, use the quiz/course context to choose a specific concept.\n"
    )
    retry_note = ""
    if attempt_number > 1:
        retry_note = "\nThe previous generated question was too similar to an existing one. Create a clearly different question now."

    if question_type == "coding":
        return f"""
Generate exactly 1 original CODING question for this course quiz.
...
Unique Generation Angle: {generation_angle}

Already existing questions:
{existing_text}

Already used concepts/signatures:
{existing_concept_text}
{retry_note}

Rules:
- Teacher Instruction has highest priority. Do not ignore it.
{requirement_rule}
- The problem must be directly based on the course, quiz title, quiz detail, chapters, or study materials.
- The main concept must be different from the already used concepts/signatures.
- Make it suitable for {difficulty} learners.
- Include a clear problem statement, input/output expectation if useful, constraints, and at least one sample.
- Starter code must be runnable Python unless the teacher asks for another language.
- Solution code must solve the problem, not print placeholder text.
- Do not repeat, reword, or slightly modify any existing question.
- Do not generate generic questions unrelated to the course.
...
"""
    ...
    return f"""
Generate exactly 1 original multiple choice question for this course quiz.
...
- Teacher Instruction has highest priority. Do not ignore it.
{requirement_rule}
- The question must be directly based on the course, quiz title, quiz detail, chapters, or study materials.
- The main concept must be different from the already used concepts/signatures.
- Make it suitable for {difficulty} learners.
- Ask about understanding, application, debugging, comparison, or scenario-based use. Avoid shallow trivia.
- Provide 4 strong options.
...
def _parse_ai_quiz_json(ai_text):
    if isinstance(ai_text, dict):
        return ai_text

    json_match = re.search(r"\{.*\}", str(ai_text or ""), re.DOTALL)
    if not json_match:
        return None
    return json.loads(json_match.group())


def _normalize_generated_quiz_payload(parsed, question_type):
    if question_type == "coding":
        return {
            "questions": str(parsed.get("questions") or parsed.get("question") or "").strip(),
    ...
    normalized = {
        "questions": str(parsed.get("questions") or parsed.get("question") or "").strip(),
        "ans1": str(parsed.get("ans1") or parsed.get("option1") or "").strip(),
        "ans2": str(parsed.get("ans2") or parsed.get("option2") or "").strip(),
        "ans3": str(parsed.get("ans3") or parsed.get("option3") or "").strip(),
        "ans4": str(parsed.get("ans4") or parsed.get("option4") or "").strip(),
        "right_ans": str(parsed.get("right_ans") or parsed.get("correct_answer") or "").strip(),
    }

    options = [normalized["ans1"], normalized["ans2"], normalized["ans3"], normalized["ans4"]]
    for option in options:
        if option.lower() == normalized["right_ans"].lower():
            normalized["right_ans"] = option
            break

    return normalized


def _validate_generated_quiz_payload(parsed, question_type):
    if not parsed.get("questions"):
        return "Generated question is empty."

    if question_type == "coding":
        if not parsed.get("coding_starter_code") or not parsed.get("coding_solution"):
            return "Coding question must include starter code and solution."
        solution_clean = _clean_quiz_text(parsed.get("coding_solution"))
        weak_solution_markers = {
            "def solve data input strip print data solve",
            "print sample solution for the given problem",
            "write your code here pass solve",
        }
        if solution_clean in weak_solution_markers:
            return "Coding solution is only a placeholder."
        if "print data" in solution_clean and len(solution_clean.split()) <= 10:
            return "Coding solution is too generic."
        if "sample solution" in solution_clean or "write your code here" in solution_clean:
            return "Coding solution contains placeholder text."
        return ""

    options = [parsed.get("ans1"), parsed.get("ans2"), parsed.get("ans3"), parsed.get("ans4")]
    if any(not option for option in options):
        return "MCQ question must include all 4 options."
    if len({_clean_quiz_text(option) for option in options}) != 4:
        return "MCQ options must be unique."
    if parsed.get("right_ans") not in options:
        return "Right answer must match exactly one option."
    return ""


def _validate_requirement_match(parsed, teacher_prompt):
    required_keywords = _extract_requirement_keywords(teacher_prompt)
    ...
    generated_text = " ".join(
        ...
        for field in [
            "questions", "ans1", "ans2", "ans3", "ans4",
            "right_ans", "coding_starter_code", "coding_solution",
        ]
    ).lower()
    matched = [word for word in required_keywords if word in generated_text]
    required_match_count = 1 if len(required_keywords) <= 3 else 2
    if len(matched) < required_match_count:
        return "Generated question did not follow the typed requirement."
    return ""


def _is_openai_quota_error(error_text):
    text = str(error_text or "").lower()
    return "insufficient_quota" in text or "exceeded your current quota" in text or "error code: 429" in text


def _local_quiz_question_fallback(quiz, question_type, teacher_prompt, reason="", course=None, existing_questions=None, difficulty="basic", focus="concept understanding"):
    topic = (getattr(quiz, "title", "") or "the quiz topic").strip()
    detail = (teacher_prompt or getattr(quiz, "detail", "") or topic).strip()
    requirement_topic = _required_requirement_topic(teacher_prompt) or detail or topic
    ...
    combined_topic = f"{requirement_topic} {topic}".lower()

    if question_type == "coding":
        if "list" in combined_topic or "array" in combined_topic:
            return {
                "questions": (
                    f"Write a Python program based on '{detail}'. Input contains n followed by n integers. "
                    "Remove duplicate values while preserving their first appearance order, then print the cleaned list as space-separated values. "
                ...
                "warning": reason or "OpenAI quota is unavailable. Template coding question generated.",
            }

        if "string" in combined_topic:
            return {
                "questions": (
                    f"Write a Python program based on '{detail}'. Read one string and count how many vowels and consonants it contains. "
                    "Ignore spaces, digits, and symbols. Print the two counts as: vowels consonants."
                ),
                "coding_starter_code": "def solve():\n    text = input().strip()\n    # Count vowels and consonants\n    \n\nsolve()",
                "coding_solution": (
                    "def solve():\n"
        ...
        if "loop" in combined_topic or "iteration" in combined_topic:
            return {
                "questions": (
                    f"Write a Python program based on '{detail}'. Read n and print the sum of all numbers from 1 to n "
                    "that are divisible by 3 or 5. If n is less than 1, print 0."
                ),
        ...
        return {
            "questions": (
                f"Write a Python program based on '{detail}'. Read n integers and print how many are even and how many are odd "
                "in the format: even odd. If n is 0, print 0 0."
            ),
    ...
    fallback_questions = [
        {
            "questions": f"Based on the requirement '{detail}', which choice best explains {requirement_topic}?",
            "ans1": f"It applies {requirement_topic} correctly in the course context",
            "ans2": "It ignores the course concept and only memorizes a term",
            "ans3": "It removes the need to understand the topic",
            "ans4": "It is unrelated to the quiz detail",
        },
        {
            "questions": f"A student is working on '{detail}'. Which action is most appropriate?",
            "ans1": f"Use {requirement_topic} to solve the scenario correctly",
            "ans2": "Choose a random answer without checking the concept",
            "ans3": "Avoid applying the topic to practical work",
            "ans4": "Use an unrelated technology instead",
        },
        {
            "questions": f"Which statement is correct about '{requirement_topic}' in this quiz?",
            "ans1": f"It must connect the typed requirement with {topic} and the assigned course material",
            "ans2": "It should be answered without course knowledge",
            "ans3": "It is always unrelated to practical usage",
            "ans4": "It has no correct application",
        },
    ]
    selected = fallback_questions[len(existing_questions) % len(fallback_questions)]
    selected["right_ans"] = selected["ans1"]
    selected["warning"] = reason or "AI service unavailable. Local fallback question generated."
    return selected


@api_view(["POST"])
def legacy_generate_single_quiz_question(request):
...
"""
    ...
    try:
        ...
        if isinstance(ai_text, dict) and "error" in ai_text:
            return Response(
                _local_quiz_question_fallback(
                    quiz,
                    question_type,
                    teacher_prompt,
                    reason=f"AI generation unavailable: {ai_text.get('error')}",
                )
            )
...
def generate_single_quiz_question(request):
    ...
    course = _get_quiz_course(quiz)
    question_type, difficulty, focus, teacher_prompt = _normalize_quiz_generation_options(request, quiz)
    ...
    existing_questions.extend(_request_generated_history(request))

    context_chunks = _build_quiz_context_chunks(quiz, course)

    try:
        last_validation_error = ""

        for attempt_number in range(1, 6):
            final_instruction = _build_quiz_generation_prompt(
                quiz=quiz,
                course=course,
            ...
            if isinstance(ai_text, dict) and "error" in ai_text:
                error_detail = str(ai_text.get("error", ""))
                if _is_openai_quota_error(error_detail):
                    fallback = _local_quiz_question_fallback(
                        quiz,
                        question_type,
                        teacher_prompt,
                        reason="OpenAI quota exceeded. Template question generated until billing/quota is fixed.",
                        course=course,
                        existing_questions=existing_questions,
                        difficulty=difficulty,
                        focus=focus,
                    ...
                    fallback["focus"] = focus
                    fallback["course_id"] = getattr(course, "id", None)
                    fallback["ai_unavailable"] = True
                    return Response(fallback)

                return Response(
                    {
                        "error": "AI generation unavailable. Please check OPENAI_API_KEY/model configuration and try again.",
                        "detail": error_detail[:300],
                    },
                    status=503,
                )

            parsed = _parse_ai_quiz_json(ai_text)
            if not parsed:
                last_validation_error = "Invalid AI format"
                continue

            parsed = _normalize_generated_quiz_payload(parsed, question_type)
            last_validation_error = _validate_generated_quiz_payload(parsed, question_type)
            if last_validation_error:
                continue

            last_validation_error = _validate_requirement_match(parsed, teacher_prompt)
            if last_validation_error:
                continue

            if _is_duplicate_quiz_question(parsed["questions"], existing_questions):
                last_validation_error = "AI generated a duplicate or very similar question."
                continue

            parsed["difficulty"] = difficulty
            parsed["focus"] = focus
            parsed["course_id"] = getattr(course, "id", None)
            return Response(parsed)

        return Response(
            {
                "error": "AI could not generate a valid unique question for this requirement. Please type a clearer concept, for example: Python list slicing, SQL inner join, React useEffect cleanup.",
