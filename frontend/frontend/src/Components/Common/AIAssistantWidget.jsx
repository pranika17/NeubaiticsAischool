...
const DEFAULT_ASSISTANT_VOICE = "marin";
const DEFAULT_ASSISTANT_LANGUAGE = "english";
...
const AIAssistantWidget = ({ role, userId, title = "AI Assistant" }) => {
  ...
  const voiceReply = true;
  const voiceName = DEFAULT_ASSISTANT_VOICE;
  const [language, setLanguage] = useState(DEFAULT_ASSISTANT_LANGUAGE);
      ...
      if (!response.ok) {
        if (response.status === 401 || response.status === 403) {
          ...
          setMessages((prev) => {
            ...
            if (lastMessage?.role === "assistant" && lastMessage?.content === "Voice auth expired. Please login again.") {
              return prev;
            }
            return [...prev, { role: "assistant", content: "Voice auth expired. Please login again." }];
          });
          setSpeaking(false);
          return;
        }
