...
const submitPayment = async () => {
  ...
  const [ratingData,setRatingData]=useState({
    ...
    const courseImage = resolveMediaUrl(courseData.featured_img, defaultCourseImageUrl);
      ...
      {relatedCourseData.map((rcourse,index) =>
        <div className='col-md-3 mb-4' key={index}>
          <div className="card">
            <Link target='__blank' to={`/detail/${rcourse.pk}`}>
              <img
                src={resolveMediaUrl(rcourse.fields.featured_img, defaultCourseImageUrl)}
                className="card-img-top fadeInUp"
                height={200}
